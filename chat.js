...
else if(inputText.toLowerCase().indexOf("girl")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("female")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("woman")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1){document.getElementById("answer").innerHTML=`Both sexes can be trained as lifeguards, the reason you observe it seems a male dominant profession is perhaps of the outdoor weather nature of the job.  Gradually, more indoor pools are being built because of lightning risks, and is also lucrative for people looking for better sun protection swimming facilities.`;}

else if(inputText.toLowerCase().indexOf("how to")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("qualif")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("require")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("prerequisite")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1){document.getElementById("answer").innerHTML=`The bronze medalion award is the minimum skill level for majority of entry level lifeguard jobs in Singapore, it does varies between companies employing them. Lifesaving 1,2,3 award is an introductory course suitable for children to appreciate water safety and own safety when dealing with other aquatic users`;}	
//*BM	
else if(inputText.toLowerCase().indexOf("how old")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1||inputText.toLowerCase().indexOf("age")>-1&&inputText.toLowerCase().indexOf("lifeguard")>-1){document.getElementById("answer").innerHTML=`Candidates taking the Bronze Medallion which is the minimum aquatic certification for lifeguards in Singapore must be age 14 on the date of examination.  But for taking a lifeguard job, most organisations require the lifeguard to be minimum age 17 to be on the job`;}	
		
else if(inputText.toLowerCase().indexOf("bm")>-1&&inputText.toLowerCase().indexOf("test")>-1||inputText.toLowerCase().indexOf("bronze medallion")>-1&&inputText.toLowerCase().indexOf("test")>-1||inputText.toLowerCase().indexOf("bronze")>-1&&inputText.toLowerCase().indexOf("medallion")>-1){document.getElementById("answer").innerHTML=`BRONZE MEDALLION<br>1.Theory test.<br>2.SLSS Resuscitation test.<br>3.Land Based Rescue (instructor-assessed).<br>4.Water Based Rescue (instructor-assessed)-conscious casualty 50m from safety.<br>5.Rescue fitness(3min15sec)-conscious casualty 50m from safety, non-contact clothing tow to support position.<br>6.Contact tow-conscious casualty 50m from safety.<br>7.Resuscitation rescue-unconscious submerge casualty 15m from safety(1min15sec support position)<br>8.Initiative-2 casualties 20m from safety.<br>9.swim 400m, min 3 different strokes(free, breast, survival, side recommended) `;}	
//*BM
//*POOL LIFEGUARD
else if(inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("award")>-1||inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("cert")>-1||inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("test")>-1){document.getElementById("answer").innerHTML=`POOL LIFEGUARD Award-brief key points<br>Minimum age: 16 years old<br>Pre-requisites: Bronze Medallion<br>1.Theory Test<br>2.Operating, maintenance, troubleshoot oxgyen resuscitators<br>3.valid SLSS CPR or Senior Resuscitation award<br>4.Contact tow-casualty 25m from safety(1min 20sec)<br>5.Resuscitation rescue-unconscious non-breathing casualty at bottom of 1.8m pool, 10m from safety.<br>Double rescue-2 casualty locked together 10m from safety.M<br>Releases-fr rear, front body, front shoulders<br>Spinal injury-2 bystanders assist<br>9.initiative-2 casualties<br>10.swim fitness-400m 11min(free, breast, side, survival  back)`;}
//*POOL LIFEGUARD	
//*AM
...
//*DA***
//*SLSS CPR**
else if(inputText.toLowerCase().indexOf("cpr")>-1&&inputText.toLowerCase().indexOf("award")>-1||inputText.toLowerCase().indexOf("resuscitation")>-1&&inputText.toLowerCase().indexOf("award")>-1||inputText.toLowerCase().indexOf("cpr")>-1&&inputText.toLowerCase().indexOf("cert")>-1){document.getElementById("answer").innerHTML=`STANDARD RESUSCITATION Award-brief key<br>1.Theory test.<br>2.CPR-demostrate on adult/child manikin(may mouth-mouth and mouth-nose)<br>3.Airway Obstruction (conscious, unconscious).<br>4.Initiative (DRS-ABC,recovery)`;}
//*SLSS CPR***
