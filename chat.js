...
else if(inputText.toLowerCase().indexOf("bm")>-1&&inputText.toLowerCase().indexOf("test")>-1||inputText.toLowerCase().indexOf("bronze medallion")>-1&&inputText.toLowerCase().indexOf("test")>-1||inputText.toLowerCase().indexOf("bronze medallion")>-1&&inputText.toLowerCase().indexOf("skills")>-1){document.getElementById("answer").innerHTML=`1.Theory test.<br>2.SLSS Resuscitation test.<br>3.Land Based Rescue (instructor-assessed).<br>4.Water Based Rescue (instructor-assessed)-conscious casualty 50m from safety.<br>5.Rescue fitness(3min15sec)-conscious casualty 50m from safety, non-contact clothing tow to support position.<br>6.Contact tow-conscious casualty 50m from safety.<br>7.Resuscitation rescue-unconscious submerge casualty 15m from safety(1min15sec support position)<br>8.Initiative-2 casualties 20m from safety.<br>9.swim 400m, min 3 different strokes(free, breast, survival, side recommended) `;}	
//*BM
//*POOL LIFEGUARD
else if(inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("award")>-1||inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("cert")>-1||inputText.toLowerCase().indexOf("lifeguard")>-1&&inputText.toLowerCase().indexOf("skills")>-1){document.getElementById("answer").innerHTML=`POOL LIFEGUARD Award-brief key points<br>Minimum age: 16 years old<br>Pre-requisites: Bronze Medallion<br>1.Theory Test<br>2.Operating, maintenance, troubleshoot oxgyen resuscitators<br>3.valid SLSS CPR or Senior Resuscitation award<br>4.Contact tow-casualty 25m from safety(1min 20sec)<br>5.Resuscitation rescue-unconscious non-breathing casualty at bottom of 1.8m pool, 10m from safety.<br>Double rescue-2 casualty locked together 10m from safety.M<br>Releases-fr rear, front body, front shoulders<br>Spinal injury-2 bystanders assist<br>9.initiative-2 casualties<br>10.swim fitness-400m 11min(free, breast, side, survival  back)`;}
//*POOL LIFEGUARD	
//*AM
